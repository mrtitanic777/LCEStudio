...
def _find_lzxc():
    """lzxc.exe lives at the lce PACKAGE root (also where PyInstaller bundles it).
    Walk up from this module so the path holds no matter how deep the module is
    nested (lce/inject.py or lce/core/inject.py) and whether we run from source or
    a frozen bundle."""
    d = _HERE
    while d:
        cand = os.path.join(d, "lzxc.exe")
        ...
        if parent == d:
            break
        d = parent
    return os.path.join(_HERE, "lzxc.exe")     # fall back to the old location
